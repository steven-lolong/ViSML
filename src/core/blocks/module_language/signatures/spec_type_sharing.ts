...
Blockly.Blocks["spec_type_sharing"] = {
  /**
   * Block for creating a list with any number of elements of any dece.
   * @this {Blockly.Block}
   */
  init: function () {
    this.appendValueInput("specBlock").setCheck("spec");
    this.appendDummyInput()
      .appendField("is sharing")
      .appendField(
    ...
    this.setMutator(new Blockly.icons.MutatorIcon(["spec_type_sharing_child"], this));
  ...
  decompose: function (workspace) {
    var containerBlock = workspace.newBlock(
      "spec_type_sharing_child_container"
    );
    containerBlock.initSvg();
    var connection = containerBlock.getInput("STACK").connection;
    for (var i = 0; i < this.itemCount_; i++) {
      var itemBlock = workspace.newBlock("spec_type_sharing_child");
      itemBlock.initSvg();
  ...
  updateShape_: function () {
    ...
    for (var i = 0; i < this.itemCount_; i++) {
      if (!this.getInput("ADD" + i)) {
        ...
        if (i > 0) {
          input.appendField(
            new Blockly.FieldLabelSerializable("and equal to"),
            "desc" + i
          );
        } else {
          input.appendField(
            new Blockly.FieldLabelSerializable("with"),
            "desc" + i
          );
        }
      }
...
Blockly.Blocks["spec_type_sharing_child"] = {
  /**
  ...
  init: function () {
    ...
    this.appendDummyInput().appendField("Long ID");
    this.setPreviousStatement(true);
    this.setNextStatement(true);
    this.setTooltip("Long ID");
    this.contextMenu = false;
  },
};

Blockly.Blocks["spec_type_sharing_child_container"] = {
  /**
  ...
  init: function () {
    this.setColour(getColorByType('signature'));
    this.appendDummyInput().appendField("Long IDs");
    this.appendStatementInput("STACK");
    this.setTooltip("Long IDs");
    this.contextMenu = false;
  },
};
// ===== Endspectype_sharing_
