...
// ======= Start fun
Blockly.Blocks["dec_fun"] = {
  init: function () {
    this.setInputsInline(false);
    this.appendDummyInput("theDoor")
      .appendField(
    ...
    this.appendDummyInput("wth")
      ...
      .setAlign(Blockly.inputs.Align.RIGHT)
      .appendField("Type variable(s)");
    this.updateShape_();
    this.setColour(getColorByType('declaration'));
    this.setOutput(true, ["dec"]);
    this.setMutator(new Blockly.icons.MutatorIcon(["fun_child"], this));
  ...
  updateTextFieldTyp: function () {
    ...
    if (this.chkTyp && !input) {
      ...
      this.moveInputBefore("inVar", "ADD0");
  ...
  decompose: function (workspace) {
    var containerBlock = workspace.newBlock("fun_child_container");
    containerBlock.initSvg();
    var connection = containerBlock.getInput("STACK").connection;
    for (var i = 0; i < this.itemCount_; i++) {
      var itemBlock = workspace.newBlock("fun_child");
      itemBlock.initSvg();
  ...
  updateShape_: function () {
    if (this.itemCount_ && this.getInput("EMPTY")) {
      ...
      this.appendDummyInput("EMPTY").appendField("").setCheck("valbind");
    }
    // Add new inputs.
    for (var i = 0; i < this.itemCount_; i++) {
...
Blockly.Blocks["fun_child"] = {
  ...
  init: function () {
    ...
    this.appendDummyInput().appendField("Clausal");
    this.setPreviousStatement(true);
    this.setNextStatement(true);
    this.setTooltip("Clausal");
    this.contextMenu = false;
  },
};

Blockly.Blocks["fun_child_container"] = {
  /**
  ...
  init: function () {
    this.setColour(getColorByType('declaration'));
    this.appendDummyInput().appendField("Clausals");
    this.appendStatementInput("STACK");
    this.setTooltip("Clausals");
    this.contextMenu = false;
  },
};
// ===== End fun
