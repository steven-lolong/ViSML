...
// let_in_end constructor
Blockly.Blocks["exp_let_in_end"] = {
  /**
   * Block for creating a let_in_end with any number of elements of any type.
   * @this {Blockly.Block}
   */
  init: function () {
    this.itemCount_ = 1;
    ...
    this.appendDummyInput("in").setAlign(Blockly.inputs.Align.RIGHT).appendField("in");
    this.setColour(getColorByType('expression'));
    this.setOutput(true, ["exp"]);
    this.setTooltip("let_in_end of expression");
    this.setMutator(new Blockly.icons.MutatorIcon(["let_in_end_child"], this));
    if (this.getInput("ADD0")) {
      this.moveInputBefore("in", "ADD0");
    }
    this.moveInputBefore("let", "in");
  },
  onchange: function () {
  ...
  decompose: function (workspace) {
    var containerBlock = workspace.newBlock("let_in_end_child_container");
    containerBlock.initSvg();
    var connection = containerBlock.getInput("STACK").connection;
    for (var i = 0; i < this.itemCount_; i++) {
      var itemBlock = workspace.newBlock("let_in_end_child");
      itemBlock.initSvg();
...
Blockly.Blocks["let_in_end_child"] = {
  /**
   * Mutator block for adding items.
   * @this {Blockly.Block}
   */
  init: function () {
    this.setColour(getColorByType('expression_sub'));
    this.appendDummyInput().appendField("expression");
    this.setPreviousStatement(true);
    this.setNextStatement(true);
    this.setTooltip("Inhabitant of expression");
    this.contextMenu = false;
  },
};

Blockly.Blocks["let_in_end_child_container"] = {
  /**
  ...
  init: function () {
    this.setColour(getColorByType('expression'));
    this.appendDummyInput().appendField("Sequence of expressions");
