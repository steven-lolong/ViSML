...
// match constructor, n > 1
Blockly.Blocks["matchs"] = {
  /**
   * Block for creating a match with any number of elements of any type.
   * @this {Blockly.Block}
   */
  init: function () {
    this.appendDummyInput().appendField("Matching");
    this.itemCount_ = 1;
    this.updateShape_();
    this.setInputsInline(false);
    this.setColour(getColorByType('expression'));
    this.setOutput(true, ["match"]);
    this.setTooltip("Match the expression");
    this.setMutator(new Blockly.icons.MutatorIcon(["match_child"], this));
  ...
  decompose: function (workspace) {
    var containerBlock = workspace.newBlock("match_child_container");
    containerBlock.initSvg();
    var connection = containerBlock.getInput("STACK").connection;
    for (var i = 0; i < this.itemCount_; i++) {
      var itemBlock = workspace.newBlock("match_child");
      itemBlock.initSvg();
  ...
  updateShape_: function () {
    ...
    while (this.getInput("EXP" + i)) {
      this.removeInput("EXP" + i);
      i++;
    }
  },
};

Blockly.Blocks["match_child"] = {
  /**
  ...
  init: function () {
    ...
    this.setTooltip("Inhabitant of matching");
    this.contextMenu = false;
  },
};

Blockly.Blocks["match_child_container"] = {
  /**
  ...
  init: function () {
    this.setColour(getColorByType('expression'));
    this.appendDummyInput().appendField("List of match");
    this.appendStatementInput("STACK");
    this.setTooltip("List of match");
    this.contextMenu = false;
  },
};
