...
Blockly.Blocks["exp_tuple"] = {
  ...
  init: function () {
    ...
    this.setTooltip("tuple of expression");
    this.setMutator(new Blockly.icons.MutatorIcon(["tuple_child"], this));
  ...
  decompose: function (workspace) {
    var containerBlock = workspace.newBlock("tuple_child_container");
    containerBlock.initSvg();
    var connection = containerBlock.getInput("STACK").connection;
    for (var i = 0; i < this.itemCount_; i++) {
      var itemBlock = workspace.newBlock("tuple_child");
      itemBlock.initSvg();
    ...
    for (var i = 0; i < this.itemCount_; i++) {
      if (!this.getInput("ADD" + i)) {
        var input = this.appendValueInput("ADD" + i)
          ...
          .setCheck(["exp"]);
        input
          .appendField(new Blockly.FieldLabelSerializable(String(i + 1)), "desc" + i)
          .appendField(
            new Blockly.FieldImage(cyan_svg, 5, 20, "*")
          );
      }
...
Blockly.Blocks["tuple_child_container"] = {
  /**
  ...
  init: function () {
    ...
    this.setTooltip("The number of inhabitant");
    this.contextMenu = false;
  },
};
